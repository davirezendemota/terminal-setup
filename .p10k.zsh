# Generated by Powerlevel10k configuration wizard on 2025-05-25 at 02:03 -03.
...
() {
  ...
  typeset -g POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=(
    ...
    git_path                # dir_relative_git
    vcs                     # git status
    docker_df               # docker stats
    # =========================[ Line #2 ]=========================
    newline                 # \n
...
function prompt_docker_df() {
  local size
  size=$(docker system df --format "{{.Size}}" 2>/dev/null | tail -n 1)

  if [[ -n "$size" ]]; then
    p10k segment -f 39 -i '🐳' -t "$size"
  fi
}

function prompt_git_path() {
  local git_root
  ...
  if [[ -z "$relative_path" ]]; then
    p10k segment -f 110 -t "$repo_name"
  else
    p10k segment -f 110 -t "$repo_name$relative_path"
  fi
}
